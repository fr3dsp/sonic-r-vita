...
static RenderVertex TrackClipEmitVert(const NearClipVert *v, float farSafe) {
    RenderVertex rv;
    int projZ = v->depth;
    if (projZ < 1) {
        projZ = 1;
    ...
    rv.sz = (float)projZ * reciprocal(farSafe);
    ...
    int fogAlpha;
    sr_double fogD = (sr_double)projZ / (sr_double)farSafe;
    if (fogD > FOG_FAR) {
        fogAlpha = 0;
    }
...
static int TrackClipAndEmitQuad(const NearClipVert v[4], int tpage, float farSafe,
    ...
    RenderVertex rv[5];
    for (int i = 0; i < n; i++) {
        rv[i] = TrackClipEmitVert(&poly[i], farSafe);
    }
    R_DrawTriFan(rv, n);
    return n - 2;
}
        ...
        for (unsigned int pi = 0; pi < polyCount; pi++) {
            ...
            if ((flags & 1) == 0) {
                ...
                if (behind == 0) {
                    ...
                    RenderVertex rv[3];
                    rv[0] = TrackClipEmitVert(&cv[0], farSafe);
                    rv[1] = TrackClipEmitVert(&cv[1], farSafe);
                    rv[2] = TrackClipEmitVert(&cv[2], farSafe);
                    R_DrawTri(rv);
                    goto next_poly;
                }
            ...
            /* Quad */
                ...
                if (behind == 0) {
                    ...
                    rv[1] = TrackClipEmitVert(&cv[1], farSafe);
                    rv[2] = TrackClipEmitVert(&cv[2], farSafe);
                    rv[3] = TrackClipEmitVert(&cv[3], farSafe);
                    R_DrawQuad(rv);
                    goto next_poly;
                }
