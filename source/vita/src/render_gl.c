...
#ifdef SONICR_VITA
extern void R_VitaFlushBatch(void);
#endif
...
int R_GetWideExtra(void)
{
    ...
    int wideUnits = (int)(((long long)fullW * 480) / fullH);
    int extra = (wideUnits - 640) / 2;
    if (extra < 0) {
        extra = 0;
    }
    if (extra > 320) {
...
void BeginFrame(void)
{
    ...
    if (wideExtra > 0) {
        int wideW = g_screenWidth + wideExtra * 2;
        offsetX = (int)(((long long)wideExtra * fullW) / wideW);
        offsetY = 0;
        vpW = (int)(((long long)g_screenWidth * fullW) / wideW);
        vpH = fullH;
    }
    ...
    glScalef(dvClipWidth / (float)(g_screenWidth + wideExtra * 2),
             -dvClipHeight / (float)g_screenHeight,
             1.0f);
    if (wideExtra > 0) {
        glTranslatef((float)wideExtra, 0.0f, 0.0f);
    }
#endif
