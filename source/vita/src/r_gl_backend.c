...
#ifdef SONICR_VITA
#define VITA_BATCH_VERTS 12288
#define VITA_FAN_MAX     64
...
static VitaVertex s_vitaBatch[VITA_BATCH_VERTS];
static int        s_vitaBatchCount = 0;

void R_VitaFlushBatch(void)
{
    if (s_vitaBatchCount == 0) {
        return;
    }
    ...
    glVertexPointer(4, GL_FLOAT, sizeof(VitaVertex), &s_vitaBatch[0].x);
    glColorPointer(4, GL_UNSIGNED_BYTE, sizeof(VitaVertex), &s_vitaBatch[0].r);
    glTexCoordPointer(2, GL_FLOAT, sizeof(VitaVertex), &s_vitaBatch[0].u);
    glDrawArrays(GL_TRIANGLES, 0, s_vitaBatchCount);

    s_vitaBatchCount = 0;
}

static int R_VitaStateClean(void)
{
    if (s_desired.textureId   != s_current.textureId   ||
        s_desired.blendMode   != s_current.blendMode   ||
        s_desired.depthTest   != s_current.depthTest   ||
        s_desired.depthFunc   != s_current.depthFunc   ||
        s_desired.depthWrite  != s_current.depthWrite  ||
        s_desired.texEnv      != s_current.texEnv      ||
        s_desired.alphaTest   != s_current.alphaTest   ||
        s_desired.alphaRef    != s_current.alphaRef    ||
        s_desired.scissorEnabled != s_current.scissorEnabled) {
        return 0;
    }
    ...
    int tp = s_desired.textureId;
    if (tp >= 0 && tp < 52 && g_tpagePixelBuf[tp] != NULL) {
        if (s_glTextureDirty[tp]) {
            return 0;
        }
        unsigned char want = s_tpageFilter[tp];
...
static void R_VitaAppendFan(const RenderVertex *v, int count)
{
    VitaVertex fan[VITA_FAN_MAX];

    if (count > VITA_FAN_MAX) {
        count = VITA_FAN_MAX;
    }
    ...
    for (int i = 0; i < count; i++) {
        ...
        fan[i].z = v[i].sz * w;
        fan[i].w = w;
        fan[i].r = (uint8_t)((argb >> 16) & 0xFF);
        fan[i].g = (uint8_t)((argb >>  8) & 0xFF);
        fan[i].b = (uint8_t)( argb        & 0xFF);
        fan[i].a = (uint8_t)((argb >> 24) & 0xFF);
        fan[i].u = v[i].u;
        fan[i].v = v[i].v;
    }

    int needed = (count - 2) * 3;
    if (s_vitaBatchCount + needed > VITA_BATCH_VERTS) {
        R_VitaFlushBatch();
    }

    VitaVertex *out = &s_vitaBatch[s_vitaBatchCount];
    for (int i = 1; i < count - 1; i++) {
        *out++ = fan[0];
        *out++ = fan[i];
        *out++ = fan[i + 1];
    }
    s_vitaBatchCount += needed;
}
#endif
